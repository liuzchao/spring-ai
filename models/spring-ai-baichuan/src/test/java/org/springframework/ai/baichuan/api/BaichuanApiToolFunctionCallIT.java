...
import org.springframework.ai.baichuan.api.BaichuanApi.ChatCompletionMessage.ToolCall;
import org.springframework.ai.baichuan.api.BaichuanApi.ChatCompletionRequest;
import org.springframework.ai.baichuan.api.BaichuanApi.ChatCompletionRequest.ToolChoiceBuilder;
import org.springframework.ai.baichuan.api.BaichuanApi.FunctionTool;
import org.springframework.ai.baichuan.api.BaichuanApi.FunctionTool.Type;
...
public class BaichuanApiToolFunctionCallIT {
	...
	private final Logger logger = LoggerFactory.getLogger(BaichuanApiToolFunctionCallIT.class);

	private final MockWeatherService weatherService = new MockWeatherService();

	private final BaichuanApi moonshotApi = new BaichuanApi(System.getenv("MOONSHOT_API_KEY"));
	...
	private void toolFunctionCall(String userMessage, String cityName) {
		...
		ChatCompletionRequest chatCompletionRequest = new ChatCompletionRequest(messages,
				BaichuanApi.ChatModel.Baichuan4_Air.getValue(), List.of(FUNCTION_TOOL), ToolChoiceBuilder.AUTO);
		...
		var functionResponseRequest = new ChatCompletionRequest(messages,
				BaichuanApi.ChatModel.Baichuan4_Air.getValue(), 0.5);

...
/**
 * Integration tests for observation instrumentation in {@link BaichuanChatModel}.
 *
 * @author Geng Rong
 */
@SpringBootTest(classes = BaichuanChatModelObservationIT.Config.class)
@EnabledIfEnvironmentVariable(named = "Baichuan_API_KEY", matches = ".+")
public class BaichuanChatModelObservationIT {

	@Autowired
	TestObservationRegistry observationRegistry;

	@Autowired
	BaichuanChatModel chatModel;
	...
	void observationForChatOperation() {
		...
		var options = BaichuanChatOptions.builder()
	...
	private void validate(ChatResponseMetadata responseMetadata) {
		TestObservationRegistryAssert.assertThat(this.observationRegistry)
			...
			.that()
			.hasContextualNameEqualTo("chat " + BaichuanApi.ChatModel.Baichuan4_Air.getValue())
			.hasLowCardinalityKeyValue(LowCardinalityKeyNames.AI_OPERATION_TYPE.asString(),
					AiOperationType.CHAT.value())
			.hasLowCardinalityKeyValue(LowCardinalityKeyNames.AI_PROVIDER.asString(), AiProvider.BAICHUAN.value())
			.hasLowCardinalityKeyValue(LowCardinalityKeyNames.REQUEST_MODEL.asString(),
					BaichuanApi.ChatModel.Baichuan4_Air.getValue())
	...
	static class Config {
		...
		public BaichuanApi baichuanApi() {
			return new BaichuanApi(System.getenv("BAICHUAN_API_KEY"));
		}

		@Bean
		public BaichuanChatModel baichuanChatModel(BaichuanApi baichuanApi,
				TestObservationRegistry observationRegistry) {
			return new BaichuanChatModel(baichuanApi, BaichuanChatOptions.builder().build(),
